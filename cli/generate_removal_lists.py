...
def run_query(client, query_def, output_dir, save_failures=True):
    ...
    if passed:
        ...
        if save_failures and query_def["check_type"] == "exact" and query_def["expected"] == 0 and count > 0:
            ...
            # Find primaryIdentifier column index from views
            views = query_def["views"]
            id_col = 0
            for i, v in enumerate(views):
                if v.endswith('.primaryIdentifier') or v == f"{query_def['model']}.primaryIdentifier":
                    id_col = i
                    break

            new_items = set()
            for row in rows:
                identifier = row[id_col] if isinstance(row, list) else str(row)
