...
try:
    ...
    print('Query #1 Returned %i - FAILED' % (len(query.rows())))
...
query.add_view(
    ...
    "allele.primaryIdentifier", "allele.symbol"
)
...
try:
    assert len(query.rows()) >= 1
    print('Query #40 Returned %i - PASSED' % (len(query.rows())))
except Exception as e:
    print('Query #40 Returned %i - FAILED' % len(query.rows()))
...
try:
    assert len(query.rows()) == 0
    print('Query #41 Returned %i - PASSED' % (len(query.rows())))
except Exception as e:
    print('Query #41 Returned %i - FAILED' % len(query.rows()))
